...
export class DivisionOverrideService {
    ...
    public async applyOverridesToDivisionData(
        ...
        return divisions.map(div => {
            ...
            // Recompute selisih so it follows the overridden portal value, not the stale aggregate
            const thumbPrint = Number(updated.thumb_print || 0);
            updated.selisih = thumbPrint > 0 ? Number(updated.total_upah_bersih || 0) - thumbPrint : 0;
            return updated;
        });
    }
}
