...
const rec = data["AB1"]?.find(r=>r.gang_code==="G1H");
console.log("rec", rec ? rec.gang_code + " wage " + rec.total_upah_kotor : "none");
if(rec){
  await db.query("DELETE FROM dbo.daftar_upah_aggregation_history WHERE period_month=7 AND period_year=2026 AND division_code='AB1' AND gang_code='G1H'");
  console.log("del ok");
